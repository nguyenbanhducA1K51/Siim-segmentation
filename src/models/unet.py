...
class Resnet(EncoderDecoder):
    def __init__(self, seg_classes=1, backbone_arch=resnet34):
...
class ResnetSuperVision(Resnet):
    ...
    def forward(self, x):
        ...
        for idx, bottleneck in enumerate(self.bottlenecks):
            ...
            print ("beffore",x.shape )
            print (self.decoder_stages[rev_idx])
            print (bottleneck)
            x = self.decoder_stages[rev_idx](x)
            print ("middle",x.shape, enc_results[rev_idx - 1].shape)
            x = bottleneck(x, enc_results[rev_idx - 1])
            print ("after",x.shape)

        if self.first_layer_stride_two:
            print (self.last_upsample)
            x = self.last_upsample(x)
            print ("last up",x.shape)
        print (self.final)
        f = self.final(x)
