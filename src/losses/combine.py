...
sys.path.append(f"{PROJECT_ROOT}/src/losses")
...
class ComboLoss(nn.Module):
    def __init__(self, weights ={
        'bce': 3,
            "dice": 1}, per_image=False, channel_weights=None, channel_losses=None):
        super().__init__()
...
class MultipleLoss(ComboLoss):
    def __init__(self, weights ={"bce":3, "dice":1}, per_image=False, channel_weights=(1, 0.5, 0.5), channel_losses=None, sv_weight=0.15):
    ...
    def forward(self, *input):
        ...
        empty_loss = F.binary_cross_entropy_with_logits(sv_outputs, sv_targets.float())
        return self.sv_weight * empty_loss + (1 - self.sv_weight) * mask_loss
        # return mask_loss