...
def preprocess(cfg):
    ...
    # fuck, the column EncodedPixels has extra space ahead in it name
    ...
    train_rle_df=train_rle_df.groupby('ImageId').agg(unique_to_list).reset_index()
...
def relocate_dicom(df,train_dicom_path ,img_dir,msk_dir,image_size=1024):
    ...
    print (f"orignial image shape {sample.shape}")
    ...
    for idx,dicom_path in enumerate(dicom_paths):
        ...
        image.save(os.path.join(img_dir,f"{img_base}.png"))
        mask= Image.fromarray(msk, mode='L')
        mask.save(os.path.join(msk_dir,f"{img_base}.png")) 
